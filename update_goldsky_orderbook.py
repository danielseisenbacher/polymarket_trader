...
COLUMNS_TO_SAVE = ['timestamp', 'maker', 'makerAssetId', 'makerAmountFilled', 'taker', 'takerAssetId',
...
def get_first_and_last_timestamp(cache_file: str) -> tuple:
    """Get the first and last timestamp from orderFilled.csv, or (0,0) if file doesn't exist or is empty"""
    ...
    try:
        ...
        if not last_line or last_line.split(",") == headers:
            #print("File contains only headers, starting from beginning of time (timestamp 0)")
            return 0, 0
        ...
        # print(f'File contains data from {first_timestamp} ({readable_first}) to {last_timestamp} ({readable_last})')
...
def redundancy_fixer(timestamps):
    ...
    for begin, end in sorted(timestamps):
        if timestamps_wo_overlapse and timestamps_wo_overlapse[-1][1] >= begin - 50:
...
def scrape(timestamp_start, timestamp_end, output_file, at_once=1000):
    ...
    while True:
        ...
        df = pd.DataFrame([flatten(x) for x in events])
...
def create_table(duck_db_path):
    """Create the markets table if it does not already exist."""
    ...
    con.close()
    print("DuckDB table 'transactions' ready.")
...
def get_latest_timestamp_from_duckdb(duck_db_path):
    ...
    result = con.execute("""
            SELECT timestamp FROM transactions ORDER BY timestamp DESC LIMIT 1;
        """).fetchone()
    ...
    if result is None:
        ...
        return 0,1748728800 # No data yet - 1.November 1761955200
    else:
        last_sync = result[0]
...
def update_goldsky_orderbook_to_duckdb(duck_db_path="polymarket.duckdb", last_time_missing_intervals=None):
    ...
    if sanity_result == "multiple_missing_intervals":
        ...
        update_goldsky_orderbook_to_duckdb(last_time_missing_intervals=last_time_missing_intervals)
